...
namespace Esparksinc\IvyPaymentGraphql\Helper;
...
class Api extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    ): array
    {
        ...
        $this->logger->debugGraphql($graphqlActionName, $orderId, 'Sent data', $data);
        ...
        try {
            ...
            $this->logger->debugGraphql($graphqlActionName, $orderId, 'Got API response exception',
                [$errorData]
            );
            throw $exception;
        } finally {
            $this->logger->debugGraphql($graphqlActionName, $orderId, 'Got API response status', [$response->getStatusCode()]);
        }
        ...
        if ($response->getStatusCode() === 200) {
            ...
            $this->logger->debugGraphql($graphqlActionName, $orderId, 'Got API response', $data);
        }
        return $data;
    }
}
